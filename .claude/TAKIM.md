...
|---|---|---|
| 2026-07-19 | **Tam Mega Sprint (5 tema)**: ekip denetimi (4 uzman paralel) → 5 kanıtlı hata + fırsat haritası → Tema 0 (para doğruluğu: resync + net/brüt KDV ~%17 kâr düzeltmesi + e-Fatura VKN), A (lot/SKT/QC), B (asistan onay katmanı + nöbetçiler), C (storefront SEO+PAYTR+kargo+kampanya motoru), D (CRM lead boru hattı). 7 migration (0023–0027), 340/340 test. Uygulama uzmanlara delegasyonla, CTO kalite kapısı + entegrasyon. **Sprint sonu: yeni ajan ihtiyacı YOK** — mevcut kadro tüm temaları karşıladı; e-Fatura canlı muhasebe-entegrasyon-uzmani'nde (dış anlaşma bekliyor). Aday liste geçerli. | Çok-modüllü sprint; CTO denetimi domain uzmanlarına dağıttı, her tema zorunlu durağından (para→finans+qa, şema→veritabani-mimari) geçti. Bir alt-görev oturum limitinde yarıda kaldı, CTO kurtardı (ders: uzman işini parça parça yeşil tutmalı). |
